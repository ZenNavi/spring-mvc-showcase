...
if (!nexacro._bInitPlatform) {
	...
	__pLayoutManager.checkValid = function (form, szContainer) {
		...
		for (i = 0; i < layout_len; i++) {
			...
			correct_type = 1;
			if (curscreentype != "desktop" && layout.mobileorientation && layout.mobileorientation != mobileorientation) {
				continue;
			}
			correct_type = 2;
			checkScreenSize();
	...
	__pWindow._on_default_sys_message = function (id) {
		if (id) {
			...
			if (arr.length > 1) {
				if (nexacro._postMessageTargetList && nexacro._postMessageTargetList.length > 0) {
					var target = nexacro._postMessageTargetList.get_item(arr[0]);

					if (target) {
						nexacro._postMessageTargetList.delete_item(arr[0]);

						target.on_after_load(arr[1]);
					}
				}
			}
		}
	...
	__pWindow._on_default_sys_dragenter = function (elem, strButton, altKey, ctrlKey, shiftKey, windowX, windowY, screenX, screenY, filelist) {
	...
	__pWindow._on_default_sys_dragover = function (elem, strButton, altKey, ctrlKey, shiftKey, windowX, windowY, screenX, screenY) {
		var dragInfo = nexacro._cur_drag_info;
	...
	__pWindow._on_default_sys_dragleave = function () {
		nexacro._cur_drag_info = null;
	};

	__pWindow._on_default_sys_drop = function (elem, strButton, altKey, ctrlKey, shiftKey, windowX, windowY, screenX, screenY) {
		var dragInfo = nexacro._cur_drag_info;
	...
	__pWindow._on_lbuttonup = function () {
		return true;
	};
		...
		if (handle) {
			...
			if (nexacro._Browser == "Runtime" && (nexacro._SystemType.toLowerCase() == "win32" || nexacro._SystemType.toLowerCase() == "win64")) {
				...
				if (this.frame && this.frame._is_click_openstatus === false) {
					if (this.frame._state_openstatus == 3) {
						if (wparam === 0) {
					...
					if (comp) {
						...
						if (prev_elem == elem) {
							...
							if (prev_elem && prev_elem.handle) {
								...
								canvasX = windowX - elem_pos[0];
								canvasY = windowY - elem_pos[1];
								from_comp._on_dragleave(prev_elem, comp, dragInfo.target, dragInfo.referTarget, dragInfo.data, dragInfo.userdata, "touch", false, false, false, canvasX, canvasY, screenX, screenY);
							}
							if (elem && elem.handle) {
								from_comp = win.findComponent(prev_elem);
								elem_pos = comp.getElement()._getPositionInWindow();
								canvasX = windowX - elem_pos[0];
								canvasY = windowY - elem_pos[1];
								comp._on_dragenter(elem, from_comp, dragInfo.target, dragInfo.referTarget, dragInfo.data, dragInfo.userdata, "touch", false, false, false, canvasX, canvasY, screenX, screenY);
							}
						}
					}
	...
	_pTouchGestureManager.ontouchend = function (win, elem, touches, changedTouches, time) {
		...
		if (is_last) {
			...
			if (up_elem && dragInfo && dragInfo.isDragging) {
				...
				if (comp) {
					comp._on_drop(up_elem, dragInfo.target, dragInfo.referTarget, dragInfo.data, dragInfo.userdata, "touch", false, false, false, up_touch.canvasx, up_touch.canvasy, up_touch.screenx, up_touch.screeny);
	...
	_pEnvironment.rtldirection = "inherit";
	...
	_pEnvironment._on_hotkey = function (keycode, altKey, ctrlKey, shiftKey, curcomp) {
		if (nexacro._enableaccessibility) {
			...
			if (forwardkey && keycode == forwardkey._keycode && 
				...
				comp = form._getNextHeadingComponent(last_comp ? last_comp : form);
				if (comp) {
				...
				var form = application.getActiveForm();
				var last_comp = form._getLastFocused();
				comp = application.getActiveForm()._getPrevHeadingComponent(last_comp ? last_comp : form);
				if (comp) {
	...
	nexacro.open = function (id, formurl, parent_frame, arr_arg, openstyles, left, top, width, height, opener, ext_openstyles) {
		...
		if (parent_frame) {
			if (!(parent_frame instanceof nexacro.Frame)) {
