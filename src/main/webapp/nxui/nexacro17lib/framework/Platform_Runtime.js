...
if (nexacro._Browser == "Runtime") {
	if (!nexacro._init_platform_runtime) {
		...
		nexacro._unregisterPopupFrame = function (id, win) {
		...
		nexacro._syshandler_onlbuttonup_forward = function (_window, elem, strButton, altKey, ctrlKey, shiftKey, windowX, windowY, screenX, screenY) {
			var ret = _window._on_sys_lbuttonup(elem, strButton, altKey, ctrlKey, shiftKey, windowX, windowY, screenX, screenY);
		...
		nexacro._syshandler_onmousewheel_forward = function (_window, elem, wheelDelta, strButton, altKey, ctrlKey, shiftKey, windowX, windowY, screenX, screenY) {
			if (!nexacro._nCountAlertConfirm > 0) {
				var ret = _window._on_sys_mousewheel(elem, 0, wheelDelta, strButton, altKey, ctrlKey, shiftKey, windowX, windowY, screenX, screenY);
				if (ret != false && ctrlKey && wheelDelta != 0) {
					...
					if (50 <= newZoom && newZoom <= 200) {
						_window._wheelZoom = newZoom;
						zoomframeElement.setElementZoom(newZoom);
						zoomframeElement.setElementSize(zoomframeElement._unscaledwidth, zoomframeElement._unscaledheight);
						if (_window._modal_frame_stack.length > 0) {
							var modalCount = _window._modal_frame_stack.length;
							var modalFrameElement;
							for (var loopI = 0; loopI < modalCount; loopI++) {
								modalFrameElement = _window._modal_frame_stack[loopI][0].getElement();
								modalFrameElement.setElementZoom(newZoom);
								modalFrameElement.setElementSize(modalFrameElement._unscaledwidth, modalFrameElement._unscaledheight, true);
							}
						}
					}
				}
		...
		nexacro._syshandler_onmousehover_forward = function (_window, elem, strButton, altKey, ctrlKey, shiftKey, windowX, windowY) {
			if (elem && elem.handle) {
		...
		nexacro._syshandler_onviewsource_forward = function (_window, elem) {
			if (elem) {
				...
				if (comp) {
					...
					while ((ref_form._type_name != "Form" && !ref_form.url) || (ref_form._type_name == "Form" && !ref_form.url && !ref_form._url)) {
		...
		nexacro.__bindEventPopupWindowLoadHandler = function (_window) {
			return function (v1, v2) {
				...
				if (_window) {
					...
					var parent_window = _application._parentwindow;
					if (parent_window) {
		...
		nexacro._createModalWindowHandle = function (parent_win, _window, name, left, top, width, height, resizable, layered, lockmode) {
			...
			var i;
			if (lockmode == 1) {
				var popupframes = nexacro._getPopupFrames();
				if (popupframes) {
		...
		nexacro._createOpenWindowHandle = function (parent_win, name, formurl, left, top, width, height, resizable, layered, taskbar, is_main, parentframe, frameopener, arr_arg) {
			...
			_window._parentwindowforopen = _application._parentwindow;
