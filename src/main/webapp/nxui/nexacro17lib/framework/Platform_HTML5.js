...
if (nexacro._Browser != "Runtime" && !nexacro._init_platform_HTML5) {
	...
	nexacro._unregisterPopupFrame = function (id, win, isparentnull) {
		if (win && win.parent && isparentnull) {
			var context = win.parent.handle;
			context.nexacro._unregisterPopupFrame(id);
			context.nexacro._getLocalStorageforService();
		}
		else {
			if (window._popupframes) {
				window._popupframes.delete_item(id);
				if (window._popupframes.length == 0) {
					window._popupframes = null;
				}
			}
		}
	};
	...
	nexacro._createPopupFrame = function (_cur_win, urlparams) {
		...
		if (parent_win) {
			...
			popupframeoption._args = parent_handle._popuparrarg;
			popupframeoption._parentframe = parent_handle._popupparentframe;
			popupframeoption._opener = parent_handle._popupframeopener;

			parent_handle._popupparentframe = null;
			parent_handle._popuparrarg = null;
			parent_handle._popupframeopener = null;
		}
		...
		nexacro._syshandler_onmousedown = function (_sysEvent, node, evt) {
			...
			if (win && elem) {
				...
				if (evt.button == nexacro_HTMLSysEvent.MOUSE_LBUTTON) {
					...
					ret = win._on_sys_lbuttondown(elem, "lbutton", evt.altKey, evt.ctrlKey, evt.shiftKey, evt.clientX, evt.clientY, evt.screenX, evt.screenY);
					...
					ret = win._on_sys_rbuttondown(elem, "rbutton", evt.altKey, evt.ctrlKey, evt.shiftKey, evt.clientX, evt.clientY, evt.screenX, evt.screenY);
					...
					ret = win._on_sys_mousedown(elem, "mbutton", evt.altKey, evt.ctrlKey, evt.shiftKey, evt.clientX, evt.clientY, evt.screenX, evt.screenY);
	...
	nexacro._syshandler_onresize = function (_sysEvent) {
		...
		if (win) {
			...
			if (w != win.clientWidth || h != win.clientHeight) {
				if ((nexacro._OS == "Android" || nexacro._Browser == "MobileSafari") && win._is_active_window) {
					var is_input_focused = false;
					var layoutmanger = nexacro._getLayoutManager();
					if (win._last_focused_elem && (win._last_focused_elem.isInputElement() || win._last_focused_elem instanceof nexacro.TextAreaElement)) {
	...
	if (!window.postMessage || (nexacro._Browser == "IE" && nexacro._BrowserVersion <= 8)) {
		...
		nexacro._postMessage = function (id) {
	...
	nexacro.__setViewportScale = function () {
		...
		if (nexacro._getDeviceName() == "iPhone") {
			if (window._linked_window == undefined) {
				if (nexacro._BrowserVersion > 10 && nexacro._BrowserVersion < 12) {
