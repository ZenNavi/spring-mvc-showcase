...
if (nexacro._Browser == "Runtime") {
	if (!nexacro._Init_systembase_runtime) {
		...
		nexacro._print = function (pThis, refform, defaultprint, valign, halign, fitonepage) {
			...
			if (elem && elem.handle) {
				...
				if (pThis._is_scrollable && elem._client_elem && elem._client_elem.handle) {
					nexacro.__print(elem._client_elem.handle, form_elem.handle, defaultprint, valign, halign, fitonepage);
				}
				else {
					nexacro.__print(elem.handle, form_elem.handle, defaultprint, valign, halign, fitonepage);
				}
			}
		};
		...
		nexacro._findclick = function (comp_name, item_comp_name, item_comp, handle) {
			var retobj = nexacro.__findclickFileUploadHandle(comp_name, item_comp_name, handle, item_comp._multiselect, item_comp);
			if (retobj) {
		...
		nexacro._download = function (url, handle, initname, targetpath) {
			...
			handle = nexacro.__downloadFileDownloadHandle(url, handle, initname, targetpath, httpheaderstr);
			if (handle == null) {
		...
		_pProgressData._parse = function (bFinal) {
			...
			while (bLoop) {
				...
				if (param_parsed == true && this._parse_mode != 2) {
					if (this._error_info[0] >= 0) {
						this._applyChange_inputDataset();
					}
					else {
						if (datasetloadtype == "onsuccess") {
							bLoop = false;
						}
					}
				}
