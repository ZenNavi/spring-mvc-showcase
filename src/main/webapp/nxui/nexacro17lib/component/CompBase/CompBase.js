...
if (!nexacro.Component) {
		...
		if (this.visible != v) {
			...
			if (control_elem) {
				...
				if (this.visible) {
					...
					this._apply_status("", this._status, "", this._userstatus);
				}
				else {
	...
	_pComponent.set_scrollbarincbuttonsize = function (scrollbarincbuttonsize) {
		...
		if (scrollbarincbuttonsize !== undefined) {
			scrollbarbaroutsize = parseInt(scrollbarincbuttonsize);
	...
	_pComponent.on_apply_prop_rtldirection = function () {
		var control_elem = this._control_element;
		if (control_elem) {
			control_elem.setElementDirection(this.rtldirection);
		}
	...
	_pComponent._on_apply_status = function (oldstatus, status, olduserstatus, userstatus, apply, is_userstatus, status_param, value_param) {
	...
	_pComponent._apply_status_toelement = function (oldstatus, status, olduserstatus, userstatus, apply, is_userstatus, status_param, value_param) {
		...
		if (this.visible && control_elem) {
			...
			this._on_apply_status(oldstatus, status, olduserstatus, userstatus, apply, is_userstatus, status_param, value_param);
		}
	};
	...
	_pComponent._adjustPosition = function () {
		...
		if (this._isRtl()) {
			this._adjust_left_ltr = calc_right;
		}
		...
		if (compid) {
			if (!this._is_subcontrol) {
				...
				if (form) {
					...
					var this_idx = comps.indexOf(this.id);
					var target_idx = comps.indexOf(compid);
				...
				if (parent._findChild) {
					...
					if (form) {
						var comps = form.components;
						var source_idx = comps.indexOf(this.id);
						var target_idx = comps.indexOf(compid);
	...
	_pComponent._getPopupContainerCSSSelector = function () {
		...
		if (popupcontrol) {
			...
			if (userclassselectors && userclassselectors.length > 0) {
				var userclassselector_str;
				while (userclassselectors && userclassselectors.length > 0) {
					userclassselector_str = userclassselectors.pop();
	...
	_pPopupControl._popup = function (left, top, width, height) {
		...
		var control_elem = this._control_element;
		var _window = this._getWindow();
		if (_window._wheelZoom != undefined) {
	...
	_pPopupControl._popupBy = function (from_comp, left, top, width, height, center) {
		...
		if (center) {
			...
			var p = nexacro._getPopupElementPositionInFrame(from_comp.getElement(), left, top);
			left = p.x;
			top = p.y;
		}
