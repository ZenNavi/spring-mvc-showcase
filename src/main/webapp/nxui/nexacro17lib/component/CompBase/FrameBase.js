...
if (!nexacro.Frame) {
	...
	_pFrame._on_titlebar_movetrack = function (x, y, dragdata, windowX, windowY) {
		...
		if (!this._is_window) {
			...
			this._move(this._left + x, this._top + y, _pos.width, _pos.height);
		}
	...
	_pChildFrame.showModal = function (str_id, _parent_frame, arr_arg, opener, callback, is_async) {
		var _win = this._getWindow();
		if (_win) {
			_win._updateWrapper(undefined, "capture");
		}
		...
		if ((_window._wheelZoom != undefined) && (_window._wheelZoom != 100)) {
			...
			nexacro._OnceCallbackTimer.callonce(this, function () {
				...
				if (_window && (_window._wheelZoom != undefined) && (_window._wheelZoom != 100)) {
					...
					if (frameElem) {
						...
						frameElem.setElementSize(frameElem._unscaledwidth, frameElem._unscaledheight, true);
					}
				}
			}, 0);
		}
