...
if (nexacro._Browser != "Runtime" && !nexacro.Element) {
			...
			if (!this._is_sys_focused) {
				...
				if (nexacro._BrowserType == "Edge") {
					if (trigger_type != "lbuttondown") {
						nexacro.__setInputDOMNodeFocus(input_handle);
						if (trigger_type != undefined && trigger_type != "lbutton") {
							var _doc = this._getRootWindowHandle();
							if (_doc) {
					...
					if (this.inputtype != "date") {
						if (nexacro._Browser != "MobileSafari") {
							if (!(nexacro._enableaccessibility && nexacro._accessibilitytype == 5)) {
								...
								nexacro.__setDOMNode_SetSelect(_doc, input_handle, end_pos, end_pos);
							}
						}
		...
		if (this.handle) {
			...
			if (width == 0 || height == 0) {
				...
				if (this.borderLeftNone || this.borderTopNone || this.borderRightNone || this.borderBottomNone) {
					nexacro.__setDOMStyle_BorderNone(this.handle.style, this.borderLeftNone, this.borderTopNone, this.borderRightNone, this.borderBottomNone, this.border);
				}
			}
		}
	...
	_pControlElement.setElementBorderNone = function (borderleft, bordertop, borderright, borderbottom) {
		if (borderleft != this.borderLeftNone || bordertop != this.borderTopNone || borderright != this.borderRightNone || borderbottom != this.borderBottomNone) {
			...
			if (this.handle) {
				nexacro.__setDOMStyle_BorderNone(this.handle.style, borderleft, bordertop, borderright, borderbottom, this.border);
			}
	...
	__pContainerElement.create = function (win) {
		...
		if (owner_elem && owner_elem.handle && !this.handle) {
			...
			nexacro.__setDOMStyle_ScrollLimitX(handle.style, this._scroll_type & 0x01);
			nexacro.__setDOMStyle_ScrollLimitY(handle.style, this._scroll_type & 0x10);
	...
	__pContainerElement.createCommandStart = function () {
		...
		if (owner_elem && !this.handle) {
			...
			if (this._use_translate_scroll) {
				str += "<div class='nexainnercontainer' id='nexacontainer' style='width : " + (this._scroll_maxwidth ? this._scroll_maxwidth : this.width) + "px; height : " + (this._scroll_maxheight ? this._scroll_maxheight : this.height) + "px;'>";
			}
			...
			if (this._use_translate_scroll) {
				...
				if (inner_handle) {
					if (this._do_translate_scroll) {
						...
						nexacro.__setDOMStyle_TranslateR(inner_handle.style, -this._scroll_left, -vpos);
					}
					else {
			...
			if (this._use_translate_scroll) {
				...
				if (inner_handle) {
					if (this._do_translate_scroll) {
						...
						nexacro.__setDOMStyle_TranslateR(inner_handle.style, -hpos, -this._scroll_top);
					}
					else {
	...
	__pMultiInnerContainerElement.appendInnerContainer = function (index, _doc) {
		...
		nexacro.__setDOMNode_ClassName(inner, "nexainnercontainer");
	...
	__pMultiInnerContainerElement.updateInnerContainerPos = function () {
		...
		if (dest_handle) {
			...
			nexacro.__setDOMStyle_Translate(dest_handle.style, -hpos, -vpos);
			...
			for (var i = 0, n = this._ext_inners.length; i < n; i++) {
				...
				nexacro.__setDOMStyle_Translate(this._ext_inners[i].style, -hpos, -ext_vpos);
			}
		}
	};
		...
		if (handle !== undefined) {
			if (name) {
				...
				if (property !== undefined) {
					if (property === null || (typeof property != "object" && property != undefined && typeof property != "function")) {
						return property;
					}
