...
if (!nexacro.Sms) {
	...
	_pSms._onreadmessagelist = function (objData) {
		if (nexacro.Device.curDevice == DeviceType.WINDOWS && typeof objData == "string") {
			var jsonObjData = JSON.parse(objData);
			if (jsonObjData) {
				var arr = new Array(jsonObjData.length);
				for (var i = 0; i < jsonObjData.length; i++) {
					var jsonOb = nexacro._executeGlobalEvalStr("(" + jsonObjData[i] + ")");
				...
				var e = new nexacro.SmsMessageListEventInfo(arr);
			...
			for (var i = 0; i < objData.length; i++) {
				var jsonOb = nexacro._executeGlobalEvalStr("(" + objData[i] + ")");
				var temp = new nexacro.SmsMessage(jsonOb.id, jsonOb.phonenumber, jsonOb.message, jsonOb.type, jsonOb.date);
				arr[i] = temp;
			}
			var e = new nexacro.SmsMessageListEventInfo(arr);
		...
		if (pcheck == true) {
			...
			if (nexacro.Device.curDevice == DeviceType.ANDROID) {
				...
				if (rootPathCheck.toLowerCase() == "%userapp%") {
					...
					var _filecache = nexacro._getFileCache(strUrl);
					if (null != _filecache) {
						EnvironmentPath = 3;
...
if (!nexacro.FileDialog) {
	nexacro.FileDialog = function (name, parent) {
		this.id = this.name = name || null;
		if (parent) {
			this.parent = parent;
		}
...
if (nexacro._OS == "iOS" && !nexacro.VirtualFile) {
	nexacro.VirtualFile = function (name, parent) {
		this.id = this.name = name || null;
		if (parent) {
			this.parent = parent;
		}
	...
	_pVirtualFile.open = function (strFileName, nOption) {
		...
		if (nexacro.Device.curDevice == DeviceType.ANDROID) {
			...
			if (rootPathCheck.toLowerCase() == "%userapp%") {
				...
				var _filecache = nexacro._getFileCache(strFileName);
				if (null != _filecache) {
	...
	_pVirtualFile.getFileList = function (strPath, strSearchExpr, constOption) {
		...
		if (nexacro.Device.curDevice == DeviceType.ANDROID) {
			...
			if (rootPathCheck.toLowerCase() == "%userapp%") {
				...
				var _filecache = nexacro._getFileCache(strPath);
				if (null != _filecache) {
					this.EnvironmentPath = 3;
	...
	_pVirtualFile._onsuccess = function (objData) {
		...
		var e = new nexacro.VirtualFileEventInfo("onsuccess", objData.reason, _textdata, _bindata, eval.call(null, objData.fileattributelist), objData.filesize, objData.fileisexist);
	...
	_pVirtualFile.toJSON = function () {
		return eval.call(null, '({"filename":"' + this.filename + '","fullpath":"' + this.fullpath + '","path":"' + this.path + '"})');
	};
...
if (nexacro._OS == "iOS" && !nexacro.VirtualFileEventInfo) {
...
if (nexacro._OS == "iOS" && !nexacro.VirtualFileErrorEventInfo) {
...
if (nexacro._OS == "iOS" && !nexacro.FileAttribute) {
...
if (!nexacro.ImagePicker) {
	...
	_pImagePicker.open = function (enumGettype, enumGetimagetype) {
		if (enumGettype != null && typeof (enumGettype) != "undefined" && (typeof (enumGettype) == "string" && ((enumGettype.toLowerCase() == "url") || (enumGettype.toLowerCase() == "imagedata"))) || ((enumGettype == 0) || (enumGettype == 1) || (enumGettype == "0") || (enumGettype == "1"))) {
		...
		if (enumGetimagetype != null && typeof (enumGetimagetype) != "undefined" && (typeof (enumGetimagetype) == "string" && ((enumGetimagetype.toUpperCase() == "JPEG") || (enumGetimagetype.toUpperCase() == "PNG"))) || ((enumGetimagetype == 0) || (enumGetimagetype == 1) || (enumGetimagetype == "0") || (enumGetimagetype == "1"))) {
...
if (!nexacro.FakeXMLHttpRequest) {
		...
		if (!this.async) {
			...
			if (jsonData) {
				...
				if (jsonData["responseAllHeaders"]) {
					var responseHeaders = JSON.parse(jsonData["responseAllHeaders"]);
				}
				if (responseHeaders) {
	...
	_pFakeXMLHttpRequest._fire_onreadystatechange = function (ret) {
		...
		if (retData.e) {
			var e = JSON.parse(nexacro.Device.decodeString(retData.e));
		}
