...
if (!nexacro.ComplexComponent) {
	...
	_pComplexComponent._onResetNCChild = function (before) {
		...
		if (this._is_nc_scroll) {
			if (this.vscrollbar && this.vscrollbar.visible) {
				this.vscrollbar.move(client_right, client_top, this._onGetNCTailControlWidth(before), client_height);
			}
			if (this.hscrollbar && this.hscrollbar.visible) {
				this.hscrollbar.move(client_left, client_bottom, client_width, this._onGetNCFootControlHeight(before));
			}
		}
		if (this._is_nc_expand) {
			if (this.expandbar && this.expandbar.visible) {
				this.expandbar.move(client_right, client_top, this._onGetNCTailControlWidth(before), client_height);
			}
		}

		if (this._is_nc_head && this._nchead) {
			this._nchead.move(client_left, client_top, client_width, this._onGetNCHeadControlHeight(before));
		}
		if (this._is_nc_foot && this._ncfoot) {
			this._ncfoot.move(client_left, client_bottom, client_width, this._onGetNCFootControlHeight(before));
		}
		if (this._is_nc_lead && this._nclead) {
			this._nclead.move(client_left, client_top, this._onGetNCLeadControlWidth(before), client_height);
		}
		if (this._is_nc_tail && this._nctail) {
			this._nctail.move(client_right, client_top, this._onGetNCTailControlWidth(before), client_height);
		}

		if (this._is_nc_leadtop && this._nclt) {
			this._nclt.move(client_left, client_top, this._onGetNCLeadControlWidth(before), this._onGetNCHeadControlHeight(before));
		}
		if (this._is_nc_leadbottom && this._nclb) {
			this._nclb.move(client_left, client_bottom, this._onGetNCLeadControlWidth(before), this._onGetNCFootControlHeight(before));
		}
		if (this._is_nc_tailtop && this._ncrt) {
			this._ncrt.move(client_left, client_top, this._onGetNCTailControlWidth(before), this._onGetNCHeadControlHeight(before));
		}
		if (this._is_nc_tailbottom && this._ncrb) {
			this._ncrb.move(client_left, client_top, this._onGetNCTailControlWidth(before), this._onGetNCFootControlHeight(before));
		}
	...
	_pComplexComponent._changeItemHScrollInfo = function (pos, trigger, ready) {
		if (this._use_scrollmanager && this._is_panel_layout && this._panel) {
			{
	...
	_pComplexComponent._onGetItemScrollPrevCount = function () {
		return this._use_scrollmanager ? this._scrollmanager._getPartItemPrevCount() : 0;
	};
	_pComplexComponent._onGetItemScrollNextCount = function () {
		return this._use_scrollmanager ? this._scrollmanager._getPartItemNextCount() : 0;
	};
	_pComplexComponent._onGetItemScrollFullStart = function () {
		return this._use_scrollmanager ? this._scrollmanager._getPartItemFullStart() : this._onGetItemScrollViewStart();
	};
	_pComplexComponent._onGetItemScrollFullCount = function () {
		return this._use_scrollmanager ? this._scrollmanager._getPartItemFullCount() : this._onGetItemScrollViewCount();
	};
