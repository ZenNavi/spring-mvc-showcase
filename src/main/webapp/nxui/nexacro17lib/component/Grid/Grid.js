...
if (!nexacro.Grid) {
	...
	_pGridCell._isCellSuppress = function (cellinfo, datarow) {
		...
		return ((disp_type != "checkboxcontrol" && disp_type != "treeitemcontrol" && disp_type != "buttoncontrol" && disp_type != "progressbarcontrol" || (disp_type == "progressbarcontrol" && this._display_text.length > 0)));
	};
	...
	_pGridCalendar._on_edit_oneditclick = function (obj, e) {
		...
		if (this._type == "system") {
			...
			if (control_elem) {
				...
				nexacro._openSystemCalendar(calendar ? calendar : this, this.value);
			}
		}
		return ret;
	};
	...
	_pGrid.set_selecttype = function (v) {
		if (this.selecttype != v) {
			...
			switch (v) {
				...
				case "multitreecell":
					this.selecttype = v;
					var newtype = this._isSelectRowType();
	...
	if (nexacro._Browser == "IE") {
		...
		_pGrid._div_max_height = 33550000;
	}
	...
	_pGrid._analyzeSuppress = function (exportFlag) {
		...
		for (var j = 0; j < cellcnt; j++) {
			...
			var rowidx, text1, text2, lvl1, lvl2, displayType, csuppinfo, psuppinfo, pdatarow, cdatarow, csupp;
					...
					if (csupp > 0 && i > 0) {
						...
						if (this._hasTree && displayType == "treeitemcontrol") {
							...
							if (cdatarow == this._rowcount - 1) {
								var csuppinfo = cellinfo._getSuppressInfo(i, true);
				...
				if (supphorztype > 0 && j > 0) {
					...
					if (this._treeCellinfo != p_cellinfo && this._treeCellinfo != cellinfo) {
						if (((supphorztype == 1 || supphorztype == 3) && cellinfo._area == "left") || ((supphorztype == 2 || supphorztype == 3) && p_cellinfo._area == "right")) {
							text1 = p_cellinfo._getDisplayText(cdatarow);
							text2 = cellinfo._getDisplayText(cdatarow);
	...
	_pGrid._moveToCell = function (type, editable, colmove, area, lastcol, showcell) {
		...
		beforeCol = this._beforebodycolpos = this._selectinfo.curcol;
		beforeRow = this._beforebodyrowpos = this._selectinfo.curdsrow;
		beforeSubrow = this._beforebodysubrowpos = this._selectinfo.cur;
		...
		if (typeof (cell_columns) == "string") {
			...
			callback = function (id) {
				for (var i = 0, n = cellinfos.length; i < n; i++) {
					...
					cellinfos[i]._blinked_status[row] = proc;
					if (cell) {
			...
			var cellinfo = this._curFormat._bodycells[cell_columns];

			callback = function (id) {
				var cell = pthis._getCurrentBodyCell(row, cell_columns);

				cellinfo._blinked_status[row] = proc;
				if (cell) {
					cell._changeUserStatus("blinked", proc);
				}
	...
	_pGrid.blinkCellByInterval = function (row, cell_columns, interval, blinkcnt) {
		...
		if (interval < 0 || !interval || blinkcnt < 0 || !blinkcnt) {
			return;
		}
		...
		if (typeof (cell_columns) == "string") {
			...
			callback = function () {
				...
				for (var i = 0, n = cellinfos.length; i < n; i++) {
					...
					cellinfos[i]._blinked_status[row] = true;
					if (cell) {
						cell._changeUserStatus("blinked", true);
					}
				}
				...
				nexacro._OnceCallbackTimer.callonce(pthis, function () {
					for (var i = 0, n = cellinfos.length; i < n; i++) {
						...
						cellinfos[i]._blinked_status[row] = false;
						if (cell) {
			...
			callback = function () {
				...
				cellinfo._blinked_status[row] = true;
				if (cell) {
					cell._changeUserStatus("blinked", true);
				}

				nexacro._OnceCallbackTimer.callonce(pthis, function () {
					cell = pthis._getCurrentBodyCell(row, cell_columns);
