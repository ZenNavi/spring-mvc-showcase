...
if (!nexacro.Combo) {
	...
	_pCombo.on_apply_prop_cssclass = function () {
		var comboedit = this.comboedit;
		if (comboedit) {
			comboedit.on_apply_cssclass();
		}

		var dropbutton = this.dropbutton;
		if (dropbutton) {
			dropbutton.on_apply_cssclass();
		}

		var combolist = this.combolist;
		if (combolist) {
			combolist.on_apply_cssclass();
		}
	};
		...
		if (propid == "value") {
			...
			if (ds) {
				...
				if (idx == -1) {
					...
					this.index = idx;
					this.text = this._getItemText(idx);
				}
			}
	...
	_pCombo.set_type = function (v) {
		var type_enum = ["dropdown", "search", "filter", "filterlike"];
	...
	_pCombo.dropdown = function () {
		...
		if (!this.isDropdown()) {
			...
			if ((this.type == "filter" || this.type == "filterlike") && ds.rowcount == 0) {
			...
			switch (type) {
				...
				case "filterlike":
					if (this.type == "filter") {
						...
						ds.set_filterstr(col + ".indexOf('" + edit_val + "') > -1");
					}
					else {
						ds.set_filterstr("");
					}
	...
	_pCombo.on_fire_sys_onmousewheel = function (wheelDeltaX, wheelDeltaY, button, alt_key, ctrl_key, shift_key, screenX, screenY, canvasX, canvasY, clientX, clientY, comp, refer_comp) {
		...
		if ((!this._is_subcontrol && this != last_focus) || (this._is_subcontrol && this.parent != last_focus)) {
			return false;
		}
	...
	_pCombo._recheckIndex = function () {
		...
		if (ds && idx > -1 && ds.getRowCount() > 0 && idx < ds.getRowCount()) {
			val = this._getItemValue(idx);
			txt = this._getItemText(idx);
		}
		else {
	...
	_pComboListControl._redrawListBoxContents = function (bScrollChange) {
	...
	_pComboPopupControl._popupAuto = function () {
		...
		if (this._type == "center") {
			...
			if (mainframe) {
				this._popupBy(mainframe, pos.left, pos.top, pos.width, pos.height, true);
			}
		}
		else {
	...
	_pComboPopupControl._getPopupPosition = function () {
		...
		if (combo && combolist && mainframe) {
			...
			var combo_elem_pos = this._getElementPosition();
		...
		else if (offsetpos && combo_elem_pos.x + combolist_size[0] > win_left + window_width) {
