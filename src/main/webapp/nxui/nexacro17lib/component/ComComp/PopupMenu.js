...
if (!nexacro.PopupMenu) {
			...
			if (ds && this.levelcolumn && this.captioncolumn && this.idcolumn) {
				...
				for (var rowlength = datarow; rowlength < len; rowlength++) {
					...
					if (level == this.level) {
						...
						if (icon && !nexacro._toBoolean(checkimg)) {
	...
	_pPopupMenu._adjustPopupPosition = function (x, y, align, win_position) {
		...
		if (win_position) {
			popup_left = popup_left * wheelZoom + win_position.x;
			popup_top = popup_top * wheelZoom + win_position.y;
		}
	...
	_pPopupMenu._trackPopup = function (obj, direction, x, y) {
		...
		if (popuptype == "center") {
			...
			if (_window && _window._wheelZoom != undefined) {
				_left = _left * _window._wheelZoom / 100;
				_top = _top * _window._wheelZoom / 100;
			}
