...
if (!nexacro.Radio) {
						...
						if (!this.readonly) {
							if (focusdir == 2) {
								...
								if (!this._isAccessibilityEnable()) {
									...
									var comp = items[this._accessibility_index];
									if (comp) {
										comp._on_focus(true);
									}
								}
	...
	_pRadio.on_apply_index = function (index) {
		...
		if (ds && code) {
			var val = ds.getColumn(index, code);
	...
	_pRadio._on_radio_onkeydown = function (obj, e) {
		...
		if (keycode == nexacro.Event.KEY_TAB) {
		...
		else if (keycode == E.KEY_LEFT || keycode == E.KEY_RIGHT) {
			var ds = this._innerdataset;
			if (!ds || this.readonly) {
				return false;
			}
			...
			var op = (keycode == E.KEY_LEFT) ? -1 : (keycode == E.KEY_RIGHT) ? 1 : undefined;

			var ret = this.on_fire_canitemchange(obj, pre_index, pre_text, pre_value, this.index, this.text, this.value);
			if (ret) {
				...
				if (row_cnt != obj.idx && row_cnt >= obj.idx && 0 <= obj.idx) {
					...
					if (obj.idx != pre_index) {
						this.on_fire_onitemchanged(radioitem, pre_index, pre_text, pre_value, this.index, this.text, this.value);
					}
				}
	...
	_pRadio._on_radioitem_onclick = function (obj) {
		...
		if (this.index != obj.index) {
			...
			if (ret) {
				this.on_fire_onitemclick(obj, post_index, post_text, post_value);
