...
if (!nexacro.Sketch) {
	...
	_pSketch.on_fire_ontouchmove = function (touchinfos, changedtouchinfos, from_comp, from_refer_comp) {
		...
		this.on_fire_onmousemove("lbutton", null, null, null, evt.screenx, evt.screeny, evt.canvasx, evt.canvasy, evt.clientx - this._screenX, evt.clienty - this._screenY, from_comp, from_refer_comp);
	};
	...
	_pSketch._moveAction = function (button, clientX, clientY) {
		if (button == "lbutton" || button == "touch") {
			...
			var wheelZoom = (_window._wheelZoom == undefined) ? 1.0 : _window._wheelZoom / 100;
	...
	_pSketch.on_fire_ontouchstart = function (touchinfos, changedtouchinfos, from_comp, from_refer_comp) {
		...
		var evt = touchinfos[0];
		this.on_fire_onlbuttondown("lbutton", null, null, null, evt.screenx, evt.screeny, evt.canvasx, evt.canvasy, evt.clientx - this._screenX, evt.clienty - this._screenY, from_comp, from_refer_comp);
	};
	...
	_pSketch._setImageValue = function (image) {
		if (image instanceof nexacro.ImageSketchBackground) {
			...
			if (!base64) {
				var _image = this._canvas_elem_bg.toDataURL();
